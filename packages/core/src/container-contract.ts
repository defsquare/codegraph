...
import { MARKER_TRAITS } from "./traits.js";
import { TRAIT_NAMES } from "./names.js";
import { SECTION_ORDER, WIRE_TRAITS } from "./wire.js";

/**
 * The half of the contract JSON Schema cannot state: what a SEQUENCE of records
 * must satisfy. Generated from core's own tables rather than written by hand,
 * so the published rules cannot drift from the code that enforces them — the
 * trait/key table below IS `WIRE_TRAITS`, printed.
 */
export function containerContract(): string {
  return [
    header(),
    sections(),
    identity(),
...
function header(): string {
  return `# Codegraph interchange contract (\`model.jsonl\`, ${SCHEMA_VERSION})
...
${SECTION_ORDER.map((tag) => `- \`"t":"${tag}"\` → [\`${tag}.record.schema.json\`](${tag}.record.schema.json)`).join("\n")}
...
function references(): string {
  const rows = Object.entries(ENTITY_REFERENCE_KEYS)
    .map(([trait, spec]) => `| \`${spec.key}\` | ${trait} | ${spec.many ? "array of surrogates" : "surrogate"} |`)
    .join("\n");
  return `
...
function traitKeys(): string {
  const rows = TRAIT_NAMES.map((trait) => {
    const keys = Object.keys(WIRE_TRAITS[trait].shape);
    const marker = MARKER_TRAITS.has(trait) ? " *(marker)*" : "";
    return `| \`${trait}\` | ${keys.length === 0 ? "—" : keys.map((key) => `\`${key}\``).join(", ")} |${marker}`;
  }).join("\n");
  return `
## 4. Trait keys
...
function selfValidating(): string {
  return `
