...
describe("loading model files", () => {
  it("loads the java fixture as one clean union", () => {
    ...
    expect(loaded.union.entities.length).toBe(179);
    expect(loaded.union.edges.length).toBe(188);
  ...
  it("loads several paths as ONE union (decision 5)", () => {
    ...
    expect(loaded.union.entities.length).toBe(358);
    // The same model twice duplicates every id — a finding, not a crash.
    expect(loaded.diagnostics.duplicateIds.length).toBe(179);
