...
describe("codegraph validate on clean output", () => {
  it("exits 0 with a clean verdict on the committed Spoon snapshot", () => {
    ...
    expect(result.stdout).toContain("179 entities");
    expect(result.stdout).toContain("188 edges");
  ...
  it("exits 0 with a clean verdict on the committed TypeScript snapshot too", () => {
    const typescript = fileURLToPath(new URL("../../../fixtures/typescript/expected/model.jsonl", import.meta.url));
    const result = invoke(["validate", typescript]);

    expect(result.code).toBe(EXIT.OK);
    expect(result.stdout).toContain("OK — every model conforms");
    expect(result.stdout).toContain("lang ts");
    expect(result.stdout).not.toContain("FAILED");
  });

  it("exits 0 with a clean verdict on the committed Elixir snapshot too", () => {
    const elixir = fileURLToPath(new URL("../../../fixtures/elixir/expected/model.jsonl", import.meta.url));
    const result = invoke(["validate", elixir]);

    expect(result.code).toBe(EXIT.OK);
    expect(result.stdout).toContain("OK — every model conforms");
    expect(result.stdout).toContain("lang ex");
    expect(result.stdout).not.toContain("FAILED");
  });
  ...
  it("--json prints one parseable object and nothing else (decision 8)", () => {
    ...
    expect(report.subject).toMatchObject({ entities: 179, stubs: 27, edges: 188 });
  });
...
describe("codegraph validate keeps the failure classes apart (decision 2)", () => {
  ...
  it("loads several models as ONE union (decision 5)", () => {
    ...
    expect(report.subject.entities).toBe(358);
