...
A profile licenses an entity by the rule `required(kind) ⊆ traits ⊆ required(kind) ∪ optional(kind)`. What a profile *is* — and why it is data rather than code — is on [Profiles](/docs/reference/metamodel/profiles/).
