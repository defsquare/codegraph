...
Every reference between concepts (edge endpoints, `parent`, `children`, `attachedTo`, `declaredType`, `candidates`) identifies an entity by its natural key. The v1 file format spells each of those as the rendered id string; the [`model.jsonl`](/docs/reference/model-jsonl/) encoding uses file-scoped integer surrogates. A surrogate is **not** identity and never crosses a file boundary.
...
Why identity is a key and not a string: [Identity](/docs/explanation/identity/).
