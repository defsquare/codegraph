...
The SQLite analysis store: a **derived, disposable cache**, never the contract. Built by [`import`](/docs/reference/cli/import/), or auto-built beside a `model.jsonl` on first use and reused afterwards; `--no-cache` bypasses it. Deleting it loses nothing.
...
Empty on a plain single-model cache; filled by [`import --at`](/docs/reference/cli/import/) and [`snapshots`](/docs/reference/cli/snapshots/). Snapshots are keyed by the **natural key**, never by surrogates: surrogates are file-scoped and two revisions number their entities independently. Names (`kind`, `provenance`) are stored as TEXT for the same reason — dictionary ids are the header's own indices and are not stable across revisions. Lifespans (appeared/disappeared) are derived at query time, never stored.
