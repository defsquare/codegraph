...
[trait vocabulary](/docs/explanation/why-traits/) is FamixNG's idea reimplemented, and
...
choice. Our [city model](/docs/explanation/city-is-a-model/) makes that literal: the
...
here](/docs/explanation/time-as-structure/) — where Gource's tree grows organically.
...
set does not get a fifth value](/docs/explanation/facts-vs-inferences/).
...
exactly. Our [navigator](/docs/explanation/navigator/) is a browser page over a
...
build](/docs/explanation/extracting-without-compiling/), and its store is a
...
store exists for that reason, and the [SQL cookbook](/docs/tutorials/sql/) is aimed at
the same user.
...
[cycle report exit code](/docs/reference/exit-codes/) is the closest it gets.
...
[explanations](/docs/explanation/explaining-bottom-up/) are written in, so an
explanation names entities, aggregates, value types and events rather than
inventing its own terms.

## Where this shows up

- [Why traits](/docs/explanation/why-traits/) — the FamixNG lineage in detail.
- [The city is a model](/docs/explanation/city-is-a-model/) — the CodeCity metaphor,
  with declared bindings.
- [Time as structure](/docs/explanation/time-as-structure/) — Gource and Tornhill,
  applied.
- [The analysis pipeline](/docs/explanation/analysis-pipeline/) — the tangle metric and
  the feedback arc set.
- [How to query model.db](/docs/how-to/query-model-db/) and the
  [SQL tutorial](/docs/tutorials/sql/) — the queryable-index idea.
- [Tutorial: finding what depends on a class](/docs/tutorials/navigator/).
