...
The nine shipped profiles are rendered in full on [Language profiles](/reference/profiles/).
...
Two bounds on that licence: a [stub](/reference/metamodel/stubs/)'s exemption from the lower bound is an entity-level fact, so it is part of what the verdict is keyed on; and the verdict is per profile, since two profiles may name the same kind under different rules. It is a reader-side optimisation and requires no encoding support.
...
`--framework spring` is accepted by [`analyze`](/reference/cli/analyze/), [`city`](/reference/cli/city/), [`domain-facts`](/reference/cli/domain-facts/) and [`explain`](/reference/cli/explain/).
