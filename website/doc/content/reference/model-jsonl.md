...
`dict.kinds` is an array of non-empty strings (profile-defined). `dict.traits`, `dict.edges` and `dict.provenance` draw from closed vocabularies — see [Traits](/reference/metamodel/traits/), [Edges](/reference/metamodel/edges/) and [Provenance](/reference/metamodel/provenance/).
