...
The code city: one flat, sorted, JSON-shaped tree. No classes, no cycles, no `Map`s — it must survive `JSON.stringify` unchanged and arrive intact in a browser. Written by [`city`](/reference/cli/city/); the same shape with an extra `replay` block is written by [`replay`](/reference/cli/replay/) and by [`history --city`](/reference/cli/history/).
...
See [City metrics](/reference/city-metrics/).
...
Why the city is a model rather than a picture: [The city is a model](/explanation/city-is-a-model/).
