...
The side-car [`explain`](/reference/cli/explain/) writes: one explanation record per operation, type and module, in the vocabulary of the Specy domain metamodel. It lives **beside** the model — `<model>.insights.jsonl` by default — and never inside it, so it can be regenerated, pointed at another model, or deleted without touching `model.jsonl` or `model.db`.
