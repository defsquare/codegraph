...
for the same reasons the [city](/explanation/city-is-a-model/) does. The analyzer
...
rule](/explanation/why-jsonl/) that a dangling reference should be unwritable
...
fact](/explanation/facts-vs-inferences/)), and **selection**. A stub is drawn
...
- [Tutorial: finding what depends on a class](/tutorials/navigator/) — the fan-in
  list, the role classification, the anchor.
- [Reference: navigator.json](/reference/artifacts/navigator-json/) — the
  artifact's shape.
- [Reference: edges](/reference/metamodel/edges/) — the nine edge kinds the roles
  are recovered from.
- [How to serve to a network](/how-to/serve-network/) — the bind choice, and
  keeping it local.
- [Facts and inferences](/explanation/facts-vs-inferences/) — why one hue is
