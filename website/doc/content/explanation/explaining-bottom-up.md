...
- [Tutorial: explaining a package](/tutorials/explain/) — dry run, estimate,
  scoped run, reading a record beside its source.
- [How to estimate and cap a run](/how-to/explain-cost/) — scoping, provider
  choice, per-level models, forcing a re-run.
- [Reference: insights.jsonl](/reference/artifacts/insights-jsonl/) — the
  side-car's record shapes.
- [Reference: domain-facts](/reference/artifacts/domain-facts/) — the joined
  dossier the context packs are built from.
- [Reference: environment](/reference/environment/) — the provider variables and
  where each is read.
