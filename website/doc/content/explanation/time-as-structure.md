...
[Gource](/explanation/prior-art/), for the idea that a repository's history is
...
set](/explanation/facts-vs-inferences/) only stays meaningful while it is small.
...
key equality, because [identity is a structured key](/explanation/identity/) —
...
corpus-wide](/explanation/extracting-without-compiling/) — the stub whitelist
...
exactly the inference the [identity rule](/explanation/identity/) forbids, and the
...
- [Tutorial: replaying a project's history](/tutorials/history-replay/) — mine,
  sample, scrub, switch to owner colours.
- [How to build a temporal store](/how-to/temporal-store/) — sampling tags or
  every N commits, resuming, composing runs.
- [How to join history with a model](/how-to/history-joins/) — hidden coupling
  and dead weight, and tuning support and confidence.
- [Reference: history.jsonl](/reference/artifacts/history-jsonl/).
- [Identity is a key](/explanation/identity/) — why lineage was nearly free.
- [Prior art](/explanation/prior-art/) — Gource, Code Maat and CodeScene.
