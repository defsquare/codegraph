...
key](/explanation/identity/), and the natural key travels on the wire too, as the
...
- [Reference: the model.jsonl format](/reference/model-jsonl/) — record types,
...
- [Reference: model.db](/reference/model-db/) and [how to query
  it](/how-to/query-model-db/) — the workbench half of the split.
- [Tutorial: querying the model with SQL](/tutorials/sql/).
- [Identity is a key](/explanation/identity/) — why a surrogate is never identity.
