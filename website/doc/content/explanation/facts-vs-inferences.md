...
- [How to get a facts-only view](/how-to/facts-only-view/) — `--declared-only`,
  `--internal-only`, and reading the view stamp in the output.
- [Reference: provenance](/reference/metamodel/provenance/) — the four values as
  the model defines them.
- [How to colour a Spring codebase by role](/how-to/spring-roles/) —
  `analyze --report wiring` and `city --framework spring`.
- [Tutorial: reading a city](/tutorials/reading-the-city/) — green arcs, red
  arcs, and what a stub building means.
- [Time as structure](/explanation/time-as-structure/) — why history facts stayed
