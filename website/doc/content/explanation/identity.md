...
build](/explanation/extracting-without-compiling/). An id *looks* like it names
...
discipline](/explanation/extracting-without-compiling/) exists to prevent, so the
...
- [Reference: identity](/reference/metamodel/identity/) — the key, the rendering
  rules, canonical order.
- [Reference: the model.jsonl format](/reference/model-jsonl/) — how the key and
  its surrogates appear on the wire.
- [Why the interchange is a line-based file](/explanation/why-jsonl/) — where the
  surrogates came from.
- [How to compare two snapshots](/how-to/compare-snapshots/) and the
  [history replay tutorial](/tutorials/history-replay/) — key equality over time.
- [Time as structure](/explanation/time-as-structure/) — lineage, and what it
  cannot follow.
