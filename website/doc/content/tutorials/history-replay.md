...
- The `gson.jsonl` from [Your first code city](/tutorials/first-city/), for the
...
[the city](/tutorials/reading-the-city/). Big *and* churning is the combination
worth looking at.
...
- [Find hidden coupling and dead weight](/how-to/history-joins/) — tuning
  `--min-support` and `--min-confidence` on your own repository.
- [Sample a repository into a temporal store](/how-to/temporal-store/) — the
  `snapshots` workflow, resuming, and composing runs.
- [Time as structure](/explanation/time-as-structure/) — why history is a
