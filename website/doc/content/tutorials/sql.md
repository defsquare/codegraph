...
[Your first code city](/tutorials/first-city/) — gson at tag
...
[navigator](/tutorials/navigator/) showed one row at a time.
...
[city's Tangles toggle](/tutorials/reading-the-city/) drew in red.
...
- [Read `model.db` from your own tool](/how-to/query-model-db/) — the tables to
  start from, for a program rather than a session.
- [The `model.db` schema](/reference/model-db/) — table by table.
