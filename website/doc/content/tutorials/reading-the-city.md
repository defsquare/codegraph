...
- **Height is lines, footprint is member count.** Those are defaults, not laws.
  You choose them; see [Build a complexity city](/how-to/complexity-city/).
...
[Querying the model with SQL](/tutorials/sql/). In gson 2.14.0 exactly 443 of
...
job gates on; see [Gate a pipeline](/how-to/ci-gate/).
{{< /callout >}}
...
- [Finding what depends on a class](/tutorials/navigator/) — the arcs, one row
  at a time, with the file and line behind each.
- [Facts and inferences](/explanation/facts-vs-inferences/) — why provenance is
  a first-class value and why the two never mix.
- [The city is a model](/explanation/city-is-a-model/) — why the renderer draws
