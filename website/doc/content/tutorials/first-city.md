...
- codegraph installed from a clone and built — see [Install](/how-to/install/).
...
- [Reading a city](/tutorials/reading-the-city/) — what the heights, colours and
  arcs actually mean, and how to check them against the analyzer.
- [Finding what depends on a class](/tutorials/navigator/) — the same model, one
  dependency at a time, with the source line that proves each one.
- [Extracting without compiling](/explanation/extracting-without-compiling/) —
