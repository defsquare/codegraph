...
public sealed class EntityExtractor(CorpusWhitelist whitelist, TypeRegistry registry)
{
    ...
    private void Type(Corpus corpus, INamedTypeSymbol type)
    {
        ...
        foreach (var member in type.GetMembers())
        {
            switch (member)
            {
                case IMethodSymbol method: Method(corpus, key, type, primary, method); break;
                case IPropertySymbol property: Property(corpus, key, property); break;
                case IFieldSymbol field: Field(corpus, key, field); break;
                case IEventSymbol evt: Event(corpus, key, evt); break;
            }
        }
    }
    ...
    private void Method(Corpus corpus, NaturalKey typeKey, INamedTypeSymbol type, SyntaxNode typePrimary, IMethodSymbol method)
    {
        ...
        if (method.IsExtensionMethod && registry.Note(method.Parameters[0].Type) is { } extended)
        {
            entity.With(Traits.TAttachedTo);
            entity.AttachedTo = extended;
        }
        Comment(entity, parts.Where(p => p is not TypeDeclarationSyntax));
        declarations.Declare(entity, method, [.. parts.Where(p => p is not TypeDeclarationSyntax), anchorNode is ParameterListSyntax ? anchorNode : null]);
    ...
    private void Property(Corpus corpus, NaturalKey typeKey, IPropertySymbol property)
    {
        ...
        declarations.Declare(entity, property, parts.ToArray());
    ...
    private void Field(Corpus corpus, NaturalKey typeKey, IFieldSymbol field)
    {
        ...
        declarations.Declare(entity, field, [.. parts, .. statements]);
    ...
    private void Event(Corpus corpus, NaturalKey typeKey, IEventSymbol evt)
    {
        ...
        declarations.Declare(entity, evt, [.. parts, .. statements]);
        foreach (var part in parts) Body(corpus, part, key, null);
    }
    ...
    private List<NaturalKey> Parameters(NaturalKey owner, IEnumerable<IParameterSymbol> parameters)
    {
        ...
        {
            var key = Ids.Parameter(owner, parameter.Name);
    ...
    private void Lambda(AnonymousFunctionExpressionSyntax lambda, NaturalKey owner, Entity? ownerEntity)
    {
        ...
        declarations.Declare(entity, symbol, lambda);
    ...
    private void LocalFunction(LocalFunctionStatementSyntax local, NaturalKey owner)
    {
        ...
        declarations.Declare(entity, symbol, local);
        entity.Parameters = Parameters(key, symbol.Parameters);
        entity.LocalVariables = [];
