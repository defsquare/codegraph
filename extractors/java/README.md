...
```bash
./build.sh --java --native      # from the repo root; needs GraalVM
```

Output: `extractors/java/dist/<rid>/codegraph-java` — one self-contained
executable, ~42 MB, ~3 ms startup, and about 13× faster than `java -jar` on a
small corpus because nothing has to warm up. GraalVM is found via
`GRAALVM_HOME`, `JAVA_HOME`, `PATH`, or an sdkman candidate
(`sdk install java 25.3.4+1.r25-graalce`).

**It cannot cross-compile.** `native-image` runs the host's linker, so a macOS
binary is built on macOS — CI builds and smoke-tests one RID per runner
(`java-native` in `.github/workflows/ci.yml`), and a `v*` tag attaches all five.

**It carries its own Java API.** ECJ resolves `java.*` against the class library
of the JVM it runs inside; an image has none. Without a replacement the extractor
does not just lose precision, it invents — `System.out.println(…)` becomes a type
named `out`. So `PlatformReferenceJar` distils the Java 17 API out of the build
JDK's `lib/ct.sym` (4,721 signature-only classes, 2.6 MB) into the jar, and the
image resolves against that. On a JVM it is never unpacked: ECJ searches the
classpath last, so the VM's own library still wins and `java -jar` is unchanged.

The binary reproduces `fixtures/java/expected/model.jsonl` byte for byte — the
smoke test in `./test.sh --java`. On corpora importing types outside `java.base`
(`java.sql.Timestamp`, say) those import edges carry provenance `derived` rather
than `declared`; the package they point at, and every entity, are identical.
PLAN.md §13.10 has the measurements and why the line is drawn at `java.base`.
...
PlatformReference      runtime: the Java API a native image has no JVM to
                       borrow. Empty on a JVM — nothing changes there.
build/PlatformReferenceJar
                       build time: ct.sym -> java-api.jar + the java.base
                       reachability metadata. Never runs during extraction.
resources/META-INF/native-image/
                       dev.codegraph/codegraph-java — agent-traced Spoon/ECJ
                       reflection, COMMITTED and reviewable.
                       …-java-platform — generated by the build, do not commit.
```
